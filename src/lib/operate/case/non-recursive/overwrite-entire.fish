function _symp_operate_case_non-recursive_overwrite-entire --description 'Overwrite the target'
	...
	echo (set_color --dim){$output_prefix}(set_color normal) (set_color --italics)'Warning'(set_color normal)': Target "'{$target_path_output}'" is not a directory'\t'Replacing with symlink' 1>&2
