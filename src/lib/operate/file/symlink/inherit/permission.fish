function _symp_operate_file_symlink_inherit_permission --description='Inherit permissions of the new parent directory'
	...
	if set -q SYMP_VERBOSE # Verbosity announcement
		echo (set_color --dim){$output_prefix}(set_color normal) "Permissions of target's parent directory: "{$new_parent_octal}
	end
	...
	if set -q SYMP_VERBOSE # Verbosity announcement
		echo (set_color --dim){$output_prefix}(set_color normal) 'File octal calculated: '{$file_octal}
	end
